...
def main():
    print_header('CaImAn CNMF-E Analysis')
    ...
    c, dview, n_processes = cm.cluster.setup_cluster(
        backend='local',
        n_processes=14,  # number of process to use, if you go out of memory try to reduce this one
        single_thread=False,
    )

    # Parameter setup
    fnames = ['/mnt/sds-hd/sd20h003/Data/MPX-Series1/MPX-015/2020-08-22/OF-4/videos/miniscope/msslice.mkv']
    results_dir = '/mnt/sds-hd/sd20h003/Data/MPX-Series1/MPX-015/2020-08-22/OF-4/caiman-analysis.lmem'
    os.makedirs(results_dir, exist_ok=True)

    filename_reorder = fnames
    fr = 30  # movie frame rate
    decay_time = 0.38  # length of a typical transient in seconds
    ...
    if pw_rigid:
        ...
        # FIXME: Motion correction plot, currently disabled
    ...
    fname_new = cm.save_memmap(fname_mc, base_name='mcmap_lomem', order='C', border_to_0=bord_px)
    ...
    opts.change_params(
        params_dict={
            ...
            'update_background_components': False,  # [orig: True] sometimes setting to False improve the results
            'min_corr': min_corr,
            'min_pnr': min_pnr,
    ...
    print_section('Computing correlation and peak to noise summary images')
    ...
    # FIXME: Interactive set disabled
    ...
    #                        will be accepted, potentially with worst quality) [orig: 0.85]
    ...
    if display_images:
        print_section('Plotting components')
    ...
    # File "/home/mklumpp/Development/CaImAn/caiman/source_extraction/cnmf/estimates.py", line 1539, in save_NWB
    # nwbfile = NWBFile(sess_desc, identifier, session_start_time, experiment_description=exp_desc)
    # File "/srv/conda/envs/caiman/lib/python3.7/site-packages/hdmf/utils.py", line 560, in func_call
    # pargs = _check_args(args, kwargs)
    # File "/srv/conda/envs/caiman/lib/python3.7/site-packages/hdmf/utils.py", line 553, in _check_args
    # raise ExceptionType(msg)
    # TypeError: NWBFile.__init__: None is not allowed for 'session_start_time' (expected 'datetime', not None)
    # cnm.estimates.save_NWB('cnm_estimates_result-nwb.nwb')
    ...
    print_task('Removing {}'.format(fname_new))
    os.remove(fname_new)
