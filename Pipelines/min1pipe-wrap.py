...
from utils.messages import print_error, print_header, print_section
from min1pipe.mpipeplot import (
    create_overview_video,
    plot_spatial_overview,
    plot_temporal_overview,
)

M1P_NSIZE = 18  # structure element for background removal
...
def open_hdf5_with_matlab_header(fname, **kwargs):
    ...
    #     + ' HDF5 schema 1.00 .'
    s = (
        'MATLAB 7.3 MAT-file, Platform: unknown-any '
        + '0.1'
        + ', Created on: '
        + now.strftime('%a %b %d %H:%M:%S %Y')
        + ' HDF5 schema 1.00 .'
    ...
    # file.
    hf = h5py.File(fname, mode='a', libver=('earliest', 'v110'), **kwargs)
    yield hf
    hf.flush()
    hf.close()
...
def videos_to_mat(vid_fnames, mat_fname):
    ...
    for v_fname in vid_fnames:
        ...
        if expected_frames_n != frame_n:
            raise Exception(
                'Read an unexpected amount of frames (expected {}, got {}'.format(expected_frames_n, frame_n)
            )
        log.info(
            'Video file contained {} frames @ {}fps {}x{}px, fmt-{}; now converted to 8bit gray type:single matrices'.format(
                frame_n, fps, width, height, fmt
            )
        )

    if frames_total != cur_total_frame_n:
        log.error(
            'Read an invalid amount of frames: {} instead of {} (video file may be corrupted)'.format(
                cur_total_frame_n, frames_total
            )
        )
    ...
    ret = subprocess.call(
        [
            ...
            'min1pipe_bwHPC({fps}, {spatialr}, {se}, {ismc}, {flag}, \'{fname_video_raw}\', \'{dir_results}\');'.format(
                ...
                ismc=ismc,
                flag=flag,
                fname_video_raw=fname_video_raw.replace('\'', '\'\''),
                dir_results=dir_results.replace('\'', '\'\''),
            ),
        ],
        cwd=ml_helper_dir,
    )
...
def main(options):
    ...
    print_header(
        'Analyzing Miniscope data: {}'.format(
            os.path.dirname(options.videos[0]).replace('/mnt/sds-hd/', '', 1)
        )
    )
    ...
    with tempfile.TemporaryDirectory(prefix='mpipe_', dir=sys_tmpdir) as tdir:
        ...
        _, fps = videos_to_mat(natsorted(options.videos), frames_raw_fname)
        log.info('Data saved to {}'.format(frames_raw_fname))

        ml_tmp_res_dir = os.path.join(sys_tmpdir, 'results')
        os.makedirs(ml_tmp_res_dir, exist_ok=True)

        print_section('MIN1PIPE Pipeline')
        run_min1pipe(fps, M1P_SPATIALR, M1P_NSIZE, M1P_ISMC, M1P_FLAG, frames_raw_fname, ml_tmp_res_dir)

        log.info('Copying result from scratch space')
        shutil.copytree(ml_tmp_res_dir, results_dir, dirs_exist_ok=True)
        ...
        if write_overview_video:
            ...
            if edl_metadata:
                mkv_metadata = dict(
                    title='{} [{}], {}: Calcium imaging signal analysis'.format(
                        edl_metadata.get('subject_id'), edl_metadata['collection_name'], experiment_date_str
                    ),
                    description=(
                        'Subject {} (experiment ID: {}, source data date: {}) '
                        + 'calcium imaging signal analysis using MIN1PIPE'
                    ).format(
                        edl_metadata.get('subject_id'), edl_metadata['collection_name'], experiment_date_str
                    ),
