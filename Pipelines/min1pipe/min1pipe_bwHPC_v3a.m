function [file_name_to_save, filename_reg] = min1pipe_bwHPC(Fsi, spatialr, se, ismc, flag, fname_video_raw, dir_results)
    ...
    MIN1PIPE_ROOT_DIR = '<dataroot>/matthias/HPC/Tools/MIN1PIPE-v3a+git/';
...
%                 end
                save(m.Properties.Source, 'corr_score', 'raw_score', '-append');
            else
        ...
        disp('Running noise suppress postprocess');
        %% movement correction postprocess %%
        %%% --------- 3rd section ---------- %%%
        nflag = 2;
        filename_reg_post = [dir_results, 'mp_reg_post.mat'];
        m = noise_suppress(m, imaxy, Fsi_new, nflag, filename_reg_post);
        disp('Noise suppress postprocess done');
